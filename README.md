...
- [TOKEN](#env-token)
  ...
  - You may use a secret in place of TOKEN
    - If TOKEN is set, it takes precedence over the secret
...
| **CHECK_CONNECTION_TIMEOUT**    | 5                        | Time in seconds for connection state check to timeout (set to higher value if you find the connection constantly evaluating to 'unstable')                                                                                                                                                                    |
...
| **CONNECT** ||Provide a [country] (`Australia`), [server] (`jp35`), [country_code] (`us`), [city] (`Hungary Budapest`) or [group] (`Onion_Over_VPN`) (note CONNECT overrides CONNECTION_FILTERS, use one or the other; you can also pass `-g\|--group` (e.g `-g p2p united_states charlotte`))        |
...
| **PORT_RANGE**                  |                          | Port range to allow/whitelist for both UDP and TCP; (e.g. `PORT_RANGE=9091 9095`)                                                                                                                                                               |
| **PORTS**                       |                          | Semicolon delimited list of ports to allow/whitelist for both UDP and TCP; (e.g `PORTS=9091;9095`)                                                                                                                                              |
| **POST_QUANTUM**                | TRUE                     | Enables or disables post-quantum encryption (TRUE/FALSE; introduced in [NordVPN v3.19.0](https://github.com/NordSecurity/nordvpn-linux/releases/tag/3.19.0))                                                                                                                                                                                           |
...
| **TECHNOLOGY**<span id="env-technology"></span> | NordLynx                 | Specify the VPN Technology to use (NordLynx/OpenVPN/NordWhisper)                                                                           |
| **TOKEN**<span id="env-token"></span> |                          | Generated from your [NordVPN account web portal](https://my.nordaccount.com/dashboard/nordvpn/). You can use the [secrets](https://docs.docker.com/engine/swarm/secrets/) feature in place of TOKEN to deliver the NordVPN token; specify the secret with the name `nordvpn_token_file` and ensure the nordvpn container can access the file path                                                                                                                     |
